...
- **The direct-source assertion is risk-based, not universal.** Query the live source when the UI does not round-trip the complete effect, when the write also touches side tables or background jobs, when service-role logic bypasses the user read path, or when a false positive is expensive. Never against a mocked DB client, and never matched on a display name.
- **Enforcement follows delivery topology.** A PR-based repo runs the proof before merge and makes it a required check where branch protection allows. Where an external agent commits straight to `main` (Lovable), a required PR check cannot fire in time: CI on `push` is an alarm, not a preventive gate, so the shipping workflow owns mandatory post-deploy verification. On failure: report the evidence, do not mark the work complete, prepare a revert for explicit approval — never auto-revert.
- **Test-data boundary made explicit.** Pre-merge mutation tests never touch production. A synthetic production write requires a designated test entity, disabled side effects, a cleanup plan, and approval for the exact write — and secondary effects (email, WhatsApp, calendar, payments, webhooks) must be inventoried first. If they cannot be suppressed safely, report that persistence could not be verified rather than manufacturing a green result.
- **Origin:** a code-review exercise where an in-scope request ("add a character counter") produced an out-of-scope refactor that broke persistence while the UI still reported success — the failure mode no existing check caught, because every check watched the visible half.
...
- Added `.playwright-mcp/` to `.gitignore` — Playwright MCP was writing screenshots/console logs into the repo root instead of the session scratchpad, risking accidental commits of debug artefacts
